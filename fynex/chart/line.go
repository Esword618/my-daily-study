...
func (r *lineRenderer) Layout(s fyne.Size) {
	r.d.W = s.Width
	r.d.H = s.Height
	...
	if r.d.isV {
		for i, obj := range r.d.ChartCtn.Objects {
			...
			line.Position1 = fyne.NewPos(s.Width/2, step*float32(i)+10)
			line.Position2 = fyne.NewPos(s.Width/2+r.d.data[i], step*float32(i)+10)

			txt :=r.d.txtCtn.Objects[i].(*canvas.Text)
			txt.Move(line.Position2)
		}
	} else {
		for i, obj := range r.d.ChartCtn.Objects {
			line := obj.(*canvas.Line)
			line.StrokeWidth = r.d.lineWidth
			line.StrokeColor = r.d.lineColor
			line.Position1 = fyne.NewPos(step*float32(i)+10, s.Height/2)
			line.Position2 = fyne.NewPos(step*float32(i)+10,

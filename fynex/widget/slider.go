...
func NewSlider(min, max float64) *Slider {
	slider := &Slider{
		...
		Step:        1,
		circleScale: 1,
		Txt:         canvas.NewText(fmt.Sprint(0), color.Black),
		MinTxt:      canvas.NewText(fmt.Sprint(0), color.Black),
		Orientation: Horizontal,
	}
	slider.circleColor = theme.ForegroundColor()
...
// func (s *Slider) ColorMax(c color.Color) *Slider {
// 	s.MaxColor = c
// 	// s.Refresh()
// 	return s
// }
...
func (s *Slider) buttonDiameter() float32 {
	return theme.Padding() * standardScale * s.circleScale
}
...
func (s *Slider) CreateRenderer() fyne.WidgetRenderer {
	...
	slide := &sliderRenderer{NewBaseRenderer(objects), track, active, thumb, s}
...
func (s *sliderRenderer) Refresh() {
	...
	s.thumb.FillColor = s.slider.circleColor
	s.track.FillColor = s.slider.trackColor
	s.active.FillColor = s.slider.activeColor
	s.slider.Txt.Color = theme.ForegroundColor()
	// s.slider.Txt.Color = color.White
	s.slider.Txt.Text = fmt.Sprint(s.slider.Caption,
		" : ", s.slider.Value)
	s.slider.MinTxt.Text = fmt.Sprintf("[%.2f, %.2f]",
		s.slider.Min, s.slider.Max)
	// s.slider.MaxTxt.Text = fmt.Sprint(s.slider.Max)

	s.slider.Txt.Color = s.slider.TxtColor
	s.slider.MinTxt.Color = s.slider.MinColor
	// s.slider.MaxTxt.Color = s.slider.MaxColor
...
func (s *sliderRenderer) Layout(size fyne.Size) {
	...
	var trackPos, activePos, thumbPos, txtPos fyne.Position
	...
	case Vertical:
		...
		// s.slider.MaxTxt.Move(trackPos)
		s.slider.MinTxt.Move(trackPos.Add(fyne.NewDelta(5, trackSize.Height)))
	case Horizontal:
		trackPos = fyne.NewPos(endPad, size.Height/2)
		trackSize = fyne.NewSize(size.Width-endPad*2, trackWidth)

		s.slider.MinTxt.Move(trackPos)
		// s.slider.MaxTxt.Move(trackPos.
		// Add(fyne.NewDelta(trackSize.Width-40, 0)))
	}
	s.track.Move(trackPos)
	s.track.Resize(trackSize)

	txtPos = trackPos.Add(fyne.NewDelta(trackSize.Width/2, trackSize.Height/2))
	s.slider.Txt.Move(txtPos)
...
func (s *sliderRenderer) MinSize() fyne.Size {
	s1, s2 := minLongSide, s.slider.buttonDiameter()+10
